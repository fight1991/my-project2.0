  ...
  <section class='sys-main'>
    ...
    <div class="query-table-common">
      ...
      <el-table class='sys-table-table' :data="resultList" border highlight-current-row size="mini" @selection-change="selectVal" :height="tabHeight">
        ...
        <el-table-column label="航班号" prop="flightNo" align="left" min-width="150" v-if="thList[3].value"></el-table-column>
        ...
        <el-table-column label="驶离时间" min-width="150" align="center" v-if="thList[5].value">
          <template slot-scope="scope">
            <div>{{scope.row.dTime | date()}}</div>
          </template>
        </el-table-column>
        <el-table-column label="海关状态" align="left" min-width="150" v-if="thList[6].value">
          <template slot-scope="scope">
            <a href="javascript:void(0)" style='color: #287fca;' @click="lookReturnInfo(scope.row.preMainDeclareId)">{{scope.row.rcptStatusValue}}</a>
          </template>
        </el-table-column>
        <el-table-column label="操作状态" prop="xmlStatusValue" align="left" min-width="150" v-if="thList[7].value"></el-table-column>
