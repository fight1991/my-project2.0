<template>
  <section class='sys-main'>
    ...
    <div class="query-header">
      <el-row :gutter="20">
        <el-col :span="6">
          <el-input size="mini" v-model="carParterForm.keywords" placeholder="零部件代码/零部件名称" maxlength="50" clearable @keyup.enter.native="search"></el-input>
        </el-col>
        <el-col :span="10">
    ...
    <div class='query-body'>
      <el-table class='sys-table-table' :data="carParterList" border highlight-current-row size="mini">
        ...
        <el-table-column label="零部件代码" min-width="150">
          <template slot-scope="scope">
            <div class='sys-td-c'>{{scope.row.codeHs}}</div>
          </template>
        </el-table-column>
  ...
  methods: {
    ...
    pageList (pagination) {
      ...
      this.$store.dispatch('ajax', {
        url: 'API@/saas-dictionary/decParam/getCarPartsList',
        data: {...this.carParterForm, page: pagination},
        isPageList: true,
        router: this.$router,
        success: (res) => {
          this.carParterList = util.isEmpty(res.result) ? [] : res.result
