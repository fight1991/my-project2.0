  ...
  <section class='sys-main'>
    ...
    <div class="query-table-common">
      ...
      <el-table class='sys-table-table' :data="resultList" border highlight-current-row size="mini" @selection-change="selectVal" :height="tabHeight">
        ...
        <el-table-column label="系统编号" prop="airPid" align="left" min-width="120" v-if="thList[0].value"></el-table-column>
        <el-table-column label="单一窗口编号" prop="swSeqId" align="left" min-width="150" v-if="thList[1].value"></el-table-column>
        <el-table-column label="申报方式" align="left" min-width="150" v-if="thList[2].value">
          <template slot-scope="scope">
            <div>{{scope.row.autoDecType}}</div>
          </template>
        </el-table-column>
        <el-table-column label="航空器注册编号" prop="aircraftNo" align="left" min-width="150" v-if="thList[3].value"></el-table-column>
        <el-table-column label="进港航班号" prop="voyageIn" align="left" min-width="150" v-if="thList[4].value"></el-table-column>
        <el-table-column label="停靠港" min-width="180" align="left" v-if="thList[5].value">
          <template slot-scope="scope">
            <div>{{scope.row.stayAirport+'-'+scope.row.stayAirportValue}}</div>
          </template>
        </el-table-column>
        <el-table-column label="航班日期" align="center" min-width="120" v-if="thList[6].value">
          <template slot-scope="scope">
            <div>{{scope.row.arrivalDate | date('yyyy-MM-dd')}}</div>
          </template>
        </el-table-column>
        <el-table-column label="海关状态" align="left" min-width="120" v-if="thList[7].value">
          <template slot-scope="scope">
            <a href="javascript:void(0)" style='color: #287fca;' @click="lookReturnInfo(scope.row.airPid)">{{scope.row.rcptStatusValue}}</a>
          </template>
        </el-table-column>
        <el-table-column label="操作状态" align="left" prop="xmlStatusValue" min-width="120" v-if="thList[8].value"></el-table-column>
        <el-table-column label="操作" width="120" align="center" fixed="right">
          <template slot-scope="scope">
            <el-button size="mini" type="text" class="list-icon-edit pad0" title="编辑" v-if="scope.row.rcptStatus !== 'REC' && scope.row.rcptStatus !== 'ACC' && scope.row.rcptStatus !== 'WMA' && scope.row.rcptStatus !== 'END'" @click="editDetail('edit',scope.row.airPid)"><i class="air-i"></i></el-button>
            <el-button size="mini" type="text" class="list-icon-look pad0" title="详情" @click="editDetail('detail',scope.row.airPid)"><i class="air-i"></i></el-button>
          </template>
        </el-table-column>
      </el-table>
      <!--分页-->
    ...
    <!-- 回执 -->
    <return-info :data="returnList" :pid="listPid" type="CDS_PORT_DECLARE" @colseTemplate="closeReturn" v-if="returnVisable"></return-info>
    <!-- 回执 end -->
  </section>
</template>
<script>
  ...
  methods: {
    ...
    pageList () {
      ...
      if (this.dates === '' || this.dates === null) {
        this.QueryForm.arrivalDateStart = ''
        this.QueryForm.arrivalDateEnd = ''
      } else {
        this.QueryForm.arrivalDateStart = util.dateFormat(this.dates[0], 'yyyy-MM-dd')
        this.QueryForm.arrivalDateEnd = util.dateFormat(this.dates[1], 'yyyy-MM-dd')
      }
      this.$store.dispatch('ajax', {
        url: 'API@/dec-common/cds/other/getCdsPortList',
        data: this.QueryForm,
        isPageList: true,
        router: this.$router,
        success: (res) => {
