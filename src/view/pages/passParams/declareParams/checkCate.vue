<template>
  <section class='sys-main'>
    ...
    <div class="query-header">
      <el-row :gutter="20">
        <el-col :span="6">
          <el-input size="mini" v-model="checkForm.keywords" placeholder="商品编码/商品名称" maxlength="50" clearable></el-input>
        </el-col>
        <el-col :span="10">
    ...
    <div class='query-body'>
      <el-table class='sys-table-table' :data="checkList" border highlight-current-row size="mini">
        ...
        <el-table-column label="类别" min-width="150">
          <template slot-scope="scope">
            <div class='sys-td-c'>{{scope.row.ciqFlag}}</div>
          </template>
        </el-table-column>
        <el-table-column label="监管条件" min-width="150">
          <template slot-scope="scope">
            <div class='sys-td-c'>{{scope.row.controlMa}}</div>
          </template>
        </el-table-column>
      </el-table>
      <!--分页-->
