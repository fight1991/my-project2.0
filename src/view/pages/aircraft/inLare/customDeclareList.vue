  ...
  <section class='airvehicle-main'>
    ...
    <div class="airvehicle-table">
      ...
      <el-table class='sys-table-table' :data="resultList" border highlight-current-row size="mini" @selection-change="selectVal" :height="tabHeight">
        ...
        <el-table-column label="编号" prop="airPid" align="left" min-width="130" v-if="thList[0].value"></el-table-column>
        <el-table-column label="航空器注册编号" prop="aircraftNo" align="left" min-width="150" v-if="thList[1].value"></el-table-column>
        <el-table-column label="进港航班号" prop="voyageIn" align="left" min-width="150" v-if="thList[2].value"></el-table-column>
        <el-table-column label="停靠港" min-width="180" align="left" v-if="thList[3].value">
          <template slot-scope="scope">
            <div>{{scope.row.stayAirport+'-'+scope.row.stayAirportValue}}</div>
          </template>
        </el-table-column>
        <el-table-column label="航班日期" align="center" min-width="120" v-if="thList[4].value">
          <template slot-scope="scope">
            <div>{{scope.row.arrivalDate | date('yyyy-MM-dd')}}</div>
          </template>
        </el-table-column>
        <el-table-column label="海关状态" align="left" min-width="120" v-if="thList[5].value">
          <template slot-scope="scope">
            <a href="javascript:void(0)" style='color: #287fca;' @click="lookReturnInfo(scope.row.airPid)">{{scope.row.rcptStatusValue}}</a>
          </template>
        </el-table-column>
        <el-table-column label="操作状态" align="left" prop="xmlStatusValue" min-width="120" v-if="thList[6].value"></el-table-column>
