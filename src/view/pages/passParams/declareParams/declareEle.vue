<template>
  <section class='sys-main'>
    ...
    <div class="query-header">
      <el-row :gutter="20">
        <el-col :span="6">
          <el-input size="mini" v-model="declareForm.keywords" placeholder="商品编码" maxlength="50" clearable></el-input>
        </el-col>
        <el-col :span="10">
    ...
    <div class='query-body'>
      <el-table class='sys-table-table' :data="declareList" border highlight-current-row size="mini">
        ...
        <el-table-column label="申报要素" min-width="150">
          <template slot-scope="scope">
            <div class='sys-td-c'>{{scope.row.element}}</div>
          </template>
        </el-table-column>
        <el-table-column label="排序" min-width="150">
          <template slot-scope="scope">
            <div class='sys-td-c'>{{scope.row.sNum}}</div>
          </template>
        </el-table-column>
      </el-table>
      <!--分页-->
