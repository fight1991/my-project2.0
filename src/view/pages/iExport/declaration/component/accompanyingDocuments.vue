...
export default {
  ...
  mounted () {
    ...
    this.iEFlag = intData.iEFlag
    this.seqNo = intData.seqNo
    this.decHead = intData.decHead
    this.decList = intData.decList
    this.accDocList = intData.accDocList
    // this.saasEdocCode = JSON.parse(window.localStorage.getItem('SAAS_EDOC_CODE')).slice(0, 10)
    if (intData.decMarkLobs.length > 0) {
      this.decMarkLobsVo = intData.decMarkLobs[0]
    }
    this.accDocVisible = intData.accDocVisible
    // 聚焦点
    this.$nextTick(_ => {
