  ...
  <section class='sys-main'>
    ...
    <el-row class='sys-header'>
      <el-form label-width="0px">
        ...
        <!-- <el-row class='mg-b-15'>
          <span @click="$router.push({name: '进境、港单证申报'})" class="airvehicle-back-btn"><i class="back-btn"></i>返回</span>
        </el-row> -->
        <!-- 返回按钮 end-->
    ...
    <el-row class="airvehicle-panel airvehicle-form">
      <el-form :label-width="isEdit?labelFormWidth.sixR:'130px'" :model="submitData" ref="submitData" :rules="rules" size="mini">
        ...
        <el-row :gutter="30" class="pd-l-15">
          ...
          <el-col :span="6">
            <el-form-item :label="isEdit?'抵达港口':'抵达港口：'" prop="arrivalPort">
              <el-select size="mini" v-model="submitData.arrivalPort" v-if="isEdit"
              ...
              <span v-else class="color-txt">{{submitData.arrivalPort}}-{{submitData.arrivalPortValue}}</span>
            </el-form-item>
          </el-col>
          <el-col :span="6">
          ...
          <el-col :span="6">
            <el-form-item :label="isEdit?'抵达时间':'抵达时间：'" prop="aTime">
              <el-date-picker  v-if="isEdit" v-model="submitData.aTime" style="width:100%" type="datetime" placeholder=""></el-date-picker>
              <span v-else class="color-txt">{{submitData.aTime | date()}}</span>
            </el-form-item>
          </el-col>
          <el-col :span="6">
            <el-form-item :label="isEdit?'停靠机位':'停靠机位：'" prop="arrivalPosition">
              <el-input size="mini" clearable v-model.number="submitData.arrivalPosition" maxlength="35" v-if="isEdit"></el-input>
              <span v-else class="color-txt">{{submitData.arrivalPosition}}</span>
            </el-form-item>
          </el-col>
          <el-col :span="6">
            <el-form-item :label="isEdit?'机组人数':'机组人数：'" prop="aircrewSum">
              <el-input size="mini" clearable v-model.number="submitData.aircrewSum" maxlength="5" v-if="isEdit"></el-input>
          ...
          <el-col :span="6">
            <el-form-item :label="isEdit?'国内旅客人数':'国内旅客人数：'" prop="localePassengerSum" :class="{'more-txt-lh': isEdit}">
              <el-input size="mini" clearable v-model="submitData.localePassengerSum" maxlength="10" v-if="isEdit"></el-input>
          ...
          <el-col :span="6">
            <el-form-item :label="isEdit?'是否发现外来有害生物':'是否发现外来有害生物：'" prop="harmfulOrganisms" :class="{'more-txt-lh': isEdit}">
              ...
              <span v-else class="color-txt" v-text="submitData.harmfulOrganisms === '1'?'是':'否'"></span>
            </el-form-item>
          </el-col>
          <el-col :span="6">
            <el-form-item :label="isEdit?'是否有有效灭蚊证明':'是否有有效灭蚊证明：'" prop="mosquitoEradication" :class="{'more-txt-lh': isEdit}">
              ...
              <span v-else class="color-txt" v-text="submitData.mosquitoEradication === '1'?'是':'否'"></span>
            </el-form-item>
          </el-col>
        </el-row>
    ...
    <el-row class="airvehicle-panel airvehicle-table">
      ...
      <el-row class="airvehicle-table-icon" v-if="isEdit">
        <el-button size="mini" @click="deletes" class="list-btns list-icon-delete"><i></i>删除</el-button>
      ...
      <el-table class='sys-table-table mg-b-30' :data="submitData.cdsAirMemberVO" :height="250" border highlight-current-row size="mini" @selection-change="selectVal">
        <el-table-column  type="selection" width="37" align="center" v-if="isEdit"></el-table-column>
  ...
  data () {
    return {
      rules: {
        ...
        arrivalPort: [{ required: true, message: '请选择抵达港口', trigger: 'change' }],
        aTime: [{ required: true, message: '请选择抵达时间', trigger: 'blur' }],
