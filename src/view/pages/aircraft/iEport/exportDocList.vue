  ...
  <section class='airvehicle-main'>
    ...
    <div class="airvehicle-table">
      ...
      <el-table class='sys-table-table' :data="resultList" border highlight-current-row size="mini" @selection-change="selectVal" :height="tabHeight">
        ...
        <el-table-column label="编号" prop="preMainDeclareId" align="left" min-width="150" v-if="thList[0].value"></el-table-column>
        <el-table-column label="航空器注册编号" prop="aircraftNo" align="left" min-width="150" v-if="thList[1].value"></el-table-column>
        <el-table-column label="航班号" prop="flightNo" align="left" min-width="150" v-if="thList[2].value"></el-table-column>
        <el-table-column label="航班日期" min-width="130" align="center" v-if="thList[3].value">
          <template slot-scope="scope">
            <div>{{scope.row.flightDate | date('yyyy-MM-dd')}}</div>
          </template>
        </el-table-column>
        <el-table-column label="驶离时间" min-width="150" align="center" v-if="thList[4].value">
          <template slot-scope="scope">
            <div>{{scope.row.dTime | date()}}</div>
          </template>
        </el-table-column>
        <el-table-column label="海关状态" align="left" min-width="150" v-if="thList[5].value">
          <template slot-scope="scope">
            <a href="javascript:void(0)" style='color: #287fca;' @click="lookReturnInfo(scope.row.preMainDeclareId)">{{scope.row.rcptStatusValue}}</a>
          </template>
        </el-table-column>
        <el-table-column label="操作状态" prop="xmlStatusValue" align="left" min-width="150" v-if="thList[6].value"></el-table-column>
  ...
  methods: {
    ...
    pageList () {
      ...
      if (this.dates === '' || this.dates === null) {
        this.QueryForm.dTimeStart = ''
        this.QueryForm.dTimeEnd = ''
      } else {
        this.QueryForm.dTimeStart = util.dateFormat(this.dates[0], 'yyyy-MM-dd')
        this.QueryForm.dTimeEnd = util.dateFormat(this.dates[1], 'yyyy-MM-dd')
      }
