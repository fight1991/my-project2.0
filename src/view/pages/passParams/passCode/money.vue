<template>
  <section class='sys-main'>
    ...
    <div class="query-header">
      <el-row :gutter="20">
        <el-col :span="6">
          <el-input size="mini" v-model="moneyForm.keywords" placeholder="币制代码/币制名称" maxlength="50" clearable @keyup.enter.native="search"></el-input>
        </el-col>
        <el-col :span="10">
    ...
    <div class='query-body'>
      <el-table class='sys-table-table' :data="moneyList" border highlight-current-row size="mini">
        ...
        <el-table-column label="币制名称" min-width="150">
          <template slot-scope="scope">
            <div class='sys-td-l'>{{scope.row.currName}}</div>
          </template>
        </el-table-column>
  ...
  methods: {
    ...
    pageList (pagination) {
      ...
      this.$store.dispatch('ajax', {
        url: 'API@/saas-dictionary/decParam/getCurrList',
        data: {...this.moneyForm, page: pagination},
        isPageList: true,
        router: this.$router,
        success: (res) => {
          this.moneyList = util.isEmpty(res.result) ? [] : res.result
